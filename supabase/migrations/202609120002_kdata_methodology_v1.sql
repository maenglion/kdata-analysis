...
with methodology as (
  ...
  from ledger.methodologies
  where code = 'KDATA_CLAIM_EVAL' and version = '1.0.0'
), gates(code, name, definition, effect, score_cap, status_cap, is_core, display_order) as (
  values
    ('G-01', '증거 식별·원문 위치', '증거번호 또는 원문 위치가 없어 제3자가 근거를 재확인할 수 없음', 'BLOCK', 39::numeric, 'insufficient', true, 1),
    ('G-02', '핵심 인과 연결', '주장과 결과 사이의 핵심 인과 연결기록이 없음', 'BLOCK', 59::numeric, 'insufficient', true, 2),
    ('G-03', '권한·행위자 직접기록', '관련 시점의 권한표·담당자·접근로그 등 직접기록이 없음', 'BLOCK', 59::numeric, 'insufficient', true, 3),
    ('G-04', '직접 상충근거 해소', '주장을 직접 부정하는 자료 또는 대안해석이 해소되지 않음', 'BLOCK', 49::numeric, 'insufficient', true, 4),
    ('G-05', '원출처 증거', 'AI 출력 또는 2차 정리만 있고 원출처 증거가 없음', 'BLOCK', 39::numeric, 'insufficient', true, 5),
    ('G-06', '관련 시점 적용성', '규정·업무분장·권한이 해당 시점에도 적용됐는지 불명확함', 'BLOCK', 59::numeric, 'insufficient', true, 6),
    ('G-07', '중복·공통기원 제거', '중복 또는 공통기원 증거를 독립 근거처럼 계산하여 재산정이 필요함', 'BLOCK', 59::numeric, 'insufficient', true, 7),
    ('G-08', '확정용 직접증거', '직접증거가 부족하여 확정에는 이르지 못하지만 핵심 사실은 지지될 수 있음', 'CAP', null::numeric, 'supported', true, 8),
    ('G-09', '조건 전제 해소', '명시된 조건 또는 전제가 해소되지 않아 조건부 이상으로 올릴 수 없음', 'CAP', null::numeric, 'conditional', false, 9)
)
...
select methodology.id, gates.code, gates.name, gates.definition, gates.effect,
       gates.score_cap, gates.status_cap, gates.is_core, gates.display_order
from methodology
cross join gates
on conflict (methodology_id, code) do nothing;
